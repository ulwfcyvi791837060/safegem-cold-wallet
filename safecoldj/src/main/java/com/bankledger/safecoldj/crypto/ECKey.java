...
public class ECKey implements Serializable {
    ...
    /**
     * A constructor variant with BigInteger pubkey. See {@link com.bankledger.safecoldj.crypto.ECKey#ECKey(java.math.BigInteger, byte[])}.
     */
...
//    }
    ...
    public String toAddress(Currency currency) {
        return Utils.toAddress(Utils.sha256hash160(pub), currency);
    }
     ...
     * usually encoded using DER format, so you want {@link com.bankledger.safecoldj.crypto.ECKey.ECDSASignature#toASN1()}
     * instead. However sometimes the independent components can be useful, for instance, if you're doing to do
     * further EC maths on them.
     *
     * @throws com.bankledger.safecoldj.crypto.KeyCrypterException if this ECKey doesn't have a private part.
     */
    public ECDSASignature sign(byte[] input) throws KeyCrypterException, PasswordException {
    ...
    /**
     * Signs the given hash and returns the R and S components as BigIntegers. In the Bitcoin protocol, they are
     * usually encoded using DER format, so you want {@link com.bankledger.safecoldj.crypto.ECKey.ECDSASignature#encodeToDER()}
     ...
     * @throws com.bankledger.safecoldj.crypto.KeyCrypterException if this ECKey doesn't have a private part.
     */
    public ECDSASignature sign(byte[] input, @Nullable KeyParameter aesKey) throws KeyCrypterException, PasswordException {
        if (FAKE_SIGNATURES)
     ...
     * @throws com.bankledger.safecoldj.crypto.KeyCrypterException if this ECKey is encrypted and no AESKey is provided or it does not decrypt the ECKey.
     */
    public String signMessage(String message, String coin) throws KeyCrypterException, PasswordException {
        return signMessage(message, coin, null);
    }
     ...
     * @throws com.bankledger.safecoldj.crypto.KeyCrypterException if this ECKey is encrypted and no AESKey is provided or it does not decrypt the ECKey.
     */
    public String signMessage(String message, String coin, @Nullable KeyParameter aesKey) throws KeyCrypterException, PasswordException {
        byte[] data = Utils.formatMessageForSigning(message, coin);
        byte[] hash = Utils.doubleDigest(data);
        byte[] sigData = signHash(hash, aesKey);
        return new String(Base64.encode(sigData), Charset.forName("UTF-8"));
    }

    public byte[] signHash(byte[] hash, @Nullable KeyParameter aesKey) throws KeyCrypterException, PasswordException {
    ...
    public static ECKey signedMessageToKey(String message, String signatureBase64) throws SignatureException {
        return signedMessageToKey(message, signatureBase64, "");
    }

    public static ECKey signedMessageToKey(String message, String signatureBase64, String coin) throws SignatureException {
        ...
        byte[] messageBytes = Utils.formatMessageForSigning(message, coin);
    ...
    /**
     * Convenience wrapper around {@link com.bankledger.safecoldj.crypto.ECKey#signedMessageToKey(String, String)}. If the key derived from the
     ...
     */
    public ECKey decrypt(KeyCrypter keyCrypter, KeyParameter aesKey) throws KeyCrypterException, PasswordException {
        Preconditions.checkNotNull(keyCrypter);
     ...
     */
    //todo: See {@link com.bankledger.safecoldj.core.Address#encrypt(KeyCrypter keyCrypter, KeyParameter aesKey)} for example usage.
