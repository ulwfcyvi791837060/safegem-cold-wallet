...
package com.bankledger.safecoldj.crypto.bip38;
...
public class Bip38 {
    ...
    public static final BigInteger n = new BigInteger(1, Utils.hexStringToBytes("fffffffffffffffffffffffffffffffebaaedce6af48a03bbfd25e8cd0364141"));
     ...
     */
    public static String encryptNoEcMultiply(CharSequence passphrase, String base58EncodedPrivateKey, Currency mCurrency) throws InterruptedException, AddressFormatException {
        DumpedPrivateKey dumpedPrivateKey = new DumpedPrivateKey(base58EncodedPrivateKey, mCurrency);
        ECKey key = dumpedPrivateKey.getKey();
        dumpedPrivateKey.clearPrivateKey();
        byte[] salt = Bip38.calculateScryptSalt(key.toAddress(mCurrency));
     ...
     * @throws InterruptedException
     */
    public static SecureCharSequence decrypt(String bip38PrivateKeyString, CharSequence passphrase, Currency currency) throws InterruptedException, AddressFormatException {
        Bip38PrivateKey bip38Key = parseBip38PrivateKey(bip38PrivateKeyString);
        if (bip38Key == null) {
            return null;
        }
        if (bip38Key.ecMultiply) {
            return decryptEcMultiply(bip38Key, passphrase, currency);
        } else {
            byte[] stretcedKeyMaterial = bip38Stretch1(passphrase, bip38Key.salt, SCRYPT_LENGTH);
            return decryptNoEcMultiply(bip38Key, stretcedKeyMaterial, currency);
        }
    }

    public static SecureCharSequence decryptEcMultiply(Bip38PrivateKey bip38Key, CharSequence passphrase, Currency mCurrency) throws InterruptedException, AddressFormatException {
        ...
        byte[] newSalt = calculateScryptSalt(ecKey.toAddress(mCurrency));
        if (!Arrays.equals(bip38Key.salt, newSalt)) {
            // The passphrase is either invalid or we are on the wrong network
            return null;
        }
        DumpedPrivateKey dumpedPrivateKey = new DumpedPrivateKey(ecKey.getPrivKeyBytes(), ecKey.isCompressed(), mCurrency);
    ...
    public static SecureCharSequence decryptNoEcMultiply(Bip38PrivateKey bip38Key, byte[] stretcedKeyMaterial, Currency mCurrency) throws AddressFormatException {
        // Derive Keys
        byte[] derivedHalf1 = new byte[32];
        System.arraycopy(stretcedKeyMaterial, 0, derivedHalf1, 0, 32);
        byte[] derivedHalf2 = new byte[32];
        ...
        byte[] newSalt = calculateScryptSalt(key.toAddress(mCurrency));
        if (!Arrays.equals(bip38Key.salt, newSalt)) {
            // The passphrase is either invalid or we are on the wrong network
            return null;
        }
        ...
        // Get SIPA format
        DumpedPrivateKey dumpedPrivateKey = new DumpedPrivateKey(key.getPrivKeyBytes(), key.isCompressed(), mCurrency);
