...
import static com.bankledger.safecoldj.script.ScriptOpCodes.*;
...
public class Script {
    ...
    public String getFromAddress(Currency mCurrency) throws ScriptException {
        if (this.chunks.size() == 2
            ...
            return Utils.toAddress(Utils.sha256hash160(this.chunks.get(1).data), mCurrency);
            ...
            if (isPay2SHScript) {
                return Utils.toP2SHAddress(Utils.sha256hash160(this.chunks.get(this.chunks.size() - 1).data), mCurrency);
            }
        }
        return null;
    }
     ...
     */
    public String getToAddress(Currency mCurrency) throws ScriptException {
        if (isSentToAddress())
            return Utils.toAddress(getPubKeyHash(), mCurrency);
        else if (isPayToScriptHash())
            return Utils.toP2SHAddress(this.getPubKeyHash(), mCurrency);
        else
    ...
    public void correctlySpends(Tx txContainingThis, long scriptSigIndex, Script scriptPubKey,
        ...
        try {
            txContainingThis = new Tx(txContainingThis.getCurrency(), txContainingThis.bitcoinSerialize());
