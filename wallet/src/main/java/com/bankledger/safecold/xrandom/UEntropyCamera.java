...
import com.bankledger.safecold.xrandom.camera.CameraManager;
        ...
        public void run() {
            cameraManager.requestPreviewFrame(new Camera.PreviewCallback() {
                ...
                public void onPreviewFrame(final byte[] data, final Camera camera) {
                    if (newestData == null) {
                        newestData = new byte[data.length];
                    }
                    newestData = Arrays.copyOf(data, Math.min(newestData.length, data.length));
                    cameraHandler.postDelayed(fetchCameraDataRunnable, 100);
                }
            });
        }
    };
